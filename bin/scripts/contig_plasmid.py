...
contig_list = (
    []
)  # list to store contig codes (e.g. '>Contig1') to prevent duplicate contigs being extracted
...
def get_contig(tab):
    ...
    while n < _n:  # will only run for number of rows in pd.df
        ...
        if (
            contig in contig_list
        ):  # if contig was already added to list, continue to next contig
